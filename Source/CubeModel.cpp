...
void CubeModel::Draw()
{
	...
	GLuint NormalMatrixLocation = glGetUniformLocation(Renderer::GetShaderProgramID(), "NormalTransform");
	glm::mat4 normalMatrix = glm::transpose(glm::inverse(GetWorldMatrix()));
	glUniformMatrix4fv(NormalMatrixLocation, 1, GL_FALSE, &normalMatrix[0][0]);

	// Set lighting variables
	GLuint WorldLightPositionLocation = glGetUniformLocation(Renderer::GetShaderProgramID(), "WorldLightPosition");
	...
	// Point light
	vec4 worldLightPosition(5.0f, 5.0f, 5.0f, 0.0f);
	vec3 lightColor(1.0f, 1.0f, 1.0f);
	vec3 lightAttenuation(0.0f, 0.0f, 0.7f);

	//materialCoeffiecients
	float ka = 0.2f;
	float kd = 0.8f;
	float ks = 0.2f;
	float n = 50.0f;

	glUniform4f(WorldLightPositionLocation, worldLightPosition.x, worldLightPosition.y, worldLightPosition.z, worldLightPosition.w);
	glUniform4f(MaterialCoefficientsLocation, ka, kd, ks, n);
